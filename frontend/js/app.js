...
async function renderDashboard() {
  ...
  const tasks = overview.tasksSummary || { total: 0, byStatus: {}, completionRate: 0, recentTasks: [] };
  const statusLabels = {
  ...
  pageRoot.innerHTML = `
    ...
    <section class="section-card" style="margin-top:20px;">
      <h3>حالة مهام المشروع</h3>
      <div class="tasks-status-grid" style="margin-top:16px;">
        ${["pending", "in-progress", "completed", "cancelled"]
          .map(
            (status) => `
          <div class="task-status-card status-${escapeHtml(status)}">
            <div class="status-icon">${statusIcons[status]}</div>
            <div class="status-count">${tasks.byStatus[status] || 0}</div>
      ...
      <div class="progress-block">
        <div class="progress-header">
          <span class="muted">نسبة الإنجاز الكلية</span>
          <strong>${tasks.completionRate}٪ (${tasks.byStatus.completed || 0} من ${tasks.total})</strong>
        </div>
        <div class="progress-track">
          <div class="progress-fill" style="width: ${tasks.completionRate}%"></div>
        </div>
      </div>
      ${
        tasks.recentTasks.length
          ...
          ${tasks.recentTasks
            .map(
              (task) => `
            <div class="task-item">
              <div class="task-item-dot dot-${escapeHtml(task.status)}"></div>
              <div class="task-item-body">
                <div class="task-item-title">${escapeHtml(task.title)}</div>
                <div class="task-item-meta">
                  <span>${escapeHtml(task.assignedTo || "—")}</span>
                  ${task.dueDate ? `<span>• الاستحقاق: ${formatDate(task.dueDate)}</span>` : ""}
                  ${task.category ? `<span>• ${escapeHtml(task.category)}</span>` : ""}
                </div>
              </div>
              <div class="task-item-status">
                <span class="${badgeClass(task.status)}">${escapeHtml(statusLabels[task.status] || task.status)}</span>
              </div>
            </div>`
            )
            .join("")}
        </div>`
          : emptyMarkup("لا توجد مهام نشطة حالياً.")
      }
    </section>
