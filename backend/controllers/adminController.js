...
function createAdminController({ store }) {
  return {
    ...
    createCertificate(req, res) {
      const message = required(["name", "expiresAt"], req.body || {});
      if (message) {
        return res.status(400).json({ message });
      }
    ...
    listTasks(req, res) {
      return res.json(store.listTasks(req.query));
    },

    createTask(req, res) {
      const message = required(["title"], req.body || {});
      if (message) {
        return res.status(400).json({ message });
      }

      return res.status(201).json(store.createTask(req.body));
    },
